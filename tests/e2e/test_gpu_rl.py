"""Single-GPU fixture RL e2e tests."""
...
def _make_dpo_yaml(tmp_path: Path, model_path: Path, data_path: Path) -> tuple[str, str]:
    recipe = {
        "name": "e2e-gpu-dpo",
        ...
        "metadata": {"author": "test", "description": "GPU DPO fixture e2e"},
    }
    config = {
    ...
    recipe_path = tmp_path / "dpo_recipe.yaml"
...
def test_dpo_preference_single_gpu(tmp_path, request, gpt2, preference_tiny):
    """DPO runs one optimizer step over the preference fixture on one CUDA device."""
