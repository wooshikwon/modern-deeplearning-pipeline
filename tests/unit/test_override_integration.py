"""SettingsLoader override 통합 테스트."""
...
def test_env_var_substitution_with_auto_cast(tmp_path, monkeypatch):
    """${VAR:default} 치환은 SettingsLoader에서 타입 변환까지 적용된다."""
...
def test_run_plan_builder_preserves_callback_configs_without_resolving(
    recipe_and_config,
    tmp_path,
):
    """builder는 callbacks YAML을 typed config로 보존하고 instance resolve는 하지 않는다."""
    recipe_path, config_path = recipe_and_config
    ...
    assert len(plan.callback_configs) == 1
    callback = plan.callback_configs[0]
    assert callback.component == "ModelCheckpoint"
    assert callback.kwargs == {"monitor": "val_loss", "save_top_k": 1}
    assert plan.command == "train"
    assert plan.mode == "sft"


def test_run_plan_distributed_intent_ignores_gpu_count_without_strategy(
    recipe_and_config,
):
    """distributed_intent는 compute.gpus가 아니라 compute.distributed를 기준으로 한다."""
    recipe_path, config_path = recipe_and_config

    multi_gpu_plan = RunPlanBuilder().inference(
        recipe_path,
        config_path,
        overrides=["config.compute.gpus=4"],
    )
    distributed_plan = RunPlanBuilder().training(
        recipe_path,
        config_path,
