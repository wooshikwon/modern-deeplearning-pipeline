...
`--format json` 글로벌 옵션을 지원한다. 기본값은 `text` (Rich 테이블).
...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `adapter.method` | str | yes | - | `lora`, `qlora`, `prefix_tuning` |
...
| `training.precision` | str | no | `"fp32"` | `fp32`, `fp16`, `bf16` |
...
임의의 Python 클래스를 선택해야 하는 영역(optimizer, scheduler, loss, head, callback)은 `_component_` 패턴을 사용한다.
...
`_component_`를 사용하지 않는 영역:
- `model`: `class_path` + `pretrained` URI 패턴 (다단계 로딩 파이프라인)
- `data`: 고정 스키마 (`source`, `fields`, `format`)
- `data.tokenizer`: 고정 스키마 (`pretrained` → `AutoTokenizer`)
- `data.augmentation.steps[]`: torchvision 전용 DSL (`type` + `params`)
- `adapter`: 고정 스키마 (`method` → if/elif 분기)
- `training`: 고정 스키마
...
- **forward 계약**: `loss` 섹션 지정 시, `model.forward(batch)` → `dict(logits=...)` 반환 필수. `loss` 생략 시 `model.training_step(batch)` → `Tensor` (스칼라 loss) 반환 필수.
- **Resume 우선순위**: `adapter_model.safetensors` → `model.safetensors` → `model.pt` 순서로 체크포인트 탐색.
- **torch.compile 모드**: `training.compile`에 `"default"`, `"reduce-overhead"`, `"max-autotune"` 지정 가능. `true`이면 `"default"`.
- **MPS 지원**: Apple Silicon에서 autocast device_type이 `"cpu"`로 폴백.
- **`local://` 보안 경고**: `local://` URI는 `weights_only=False`로 pickle을 실행한다. 신뢰할 수 없는 소스의 `.pt` 파일은 사용하지 말 것.
...
### 9. `_component_` 영역에서 `type:` 키를 쓰지 말 것

optimizer, scheduler, loss, head, callback에서는 반드시 `_component_`를 사용한다. `type:` 키는 지원하지 않는다.
...
단, `data.augmentation.steps[]`는 torchvision 전용 DSL이므로 `type` + `params` 형식을 사용한다 (위 "data.augmentation 형식" 참조).
...
```json
{
  ...
  "monitoring": {
    "baseline_saved": true,
    "baseline_path": "./checkpoints/vit-lora-cifar10/baseline.json"
  }
}
```

Key fields for agent decision-making:
- `metrics`: 마지막 검증의 메트릭. 이전 실험과 비교하여 개선 여부 판단
- `stopped_reason`: `completed`, `early_stopped`, `max_steps_reached` 중 하나
  ...
  "tasks": [
    {
      ...
      "description": "이미지를 보고 클래스 판별",
      "default_head": "ClassificationHead",
      "default_metric": "accuracy",
      "compatible_models": ["vit-base-patch16-224", "resnet-50", ...]
    }
  ]
}
```
