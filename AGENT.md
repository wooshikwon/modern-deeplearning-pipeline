...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `data.dataset` | dict | yes | - | `_component_` 패턴 사용 |
| `data.augmentation` | dict | no | null | 비전 태스크에서 사용 |
...
| `data.dataloader.collate_fn` | dict | no | null | 커스텀 collate 함수 |

Dataset alias: `ImageFolder` (`mdp.data.image_folder.ImageFolderDataset`), `HuggingFace` (`mdp.data.huggingface.HuggingFaceDataset`), `CsvDataset` (`mdp.data.csv_dataset.CSVDataset`)
...
| Alias | Full Path |
|-------|-----------|
| `ImageFolder` | `mdp.data.image_folder.ImageFolderDataset` |
| `HuggingFace` | `mdp.data.huggingface.HuggingFaceDataset` |
| `CsvDataset` | `mdp.data.csv_dataset.CSVDataset` |
...
- `adapter.method: prefix_tuning` -> `r` 필수 (prefix 길이로 해석)
...
- `data.dataset._component_: HuggingFace` -> `name` 필수, `path` 불필요
- `data.dataset._component_: ImageFolder / CsvDataset / WebDataset` -> `path` 필수, `name` 불필요
- `data.tokenizer` -> 언어 태스크(`text_*`, `seq2seq`, `vision_language`)에서만 의미 있음
...
data:
  dataset:
    _component_: HuggingFace
    name: cifar10
  augmentation:
    _component_: ImageFolder
    train:
      - {_component_: RandomResizedCrop, size: 224}
      - {_component_: RandomHorizontalFlip}
      - {_component_: Normalize, mean: [0.485, 0.456, 0.406], std: [0.229, 0.224, 0.225]}
    val:
      - {_component_: Resize, size: 256}
      - {_component_: CenterCrop, size: 224}
      - {_component_: Normalize, mean: [0.485, 0.456, 0.406], std: [0.229, 0.224, 0.225]}
  dataloader:
    batch_size: 64
    num_workers: 4
