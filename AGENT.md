...
MDP의 로깅은 두 층위로 나뉜다: **MLflow layer**(실험 DB — params/metrics/tags)와 **Python logger + stdout layer**(운영·디버깅용 텍스트 출력). 두 layer는 관심사가 다르므로 설정도 독립적이다. MLflow layer 규약은 아래 `## MLflow Logging Conventions` 섹션, Python logger layer(System Logging)는 아래 "System Logging" 하위 섹션에서 navigation을 제공한다.

### System Logging

MDP는 실행 시 다음을 **자동 설정**한다 — 사용자가 recipe나 환경변수를 건드리지 않아도 운영 기본값은 "조용한 로그":

- MDP 로거 rank-0 필터링 (`Rank0Filter`, DDP 4 rank 중복 제거)
- `httpx`/`transformers`/`datasets`/`urllib3` logger INFO → WARNING
- HF 무해 warning 화이트리스트 suppression (`use_cache=True` 자동 조정, `additional_chat_templates` 404 등)
- Non-rank-0 `transformers` tqdm 비활성화 (`Loading weights` bar 중복 제거)
- 실행 시작·종료 배너 + step progress 텍스트 로그 (nohup/redirect 환경 가시성)
- OOM 발생 시 rank별 memory summary 자동 덤프
- Strategy 조건부 로깅 (DDP run에서 "FSDP shard 미적용 의심" 경고 차단)

Recipe `monitoring` 섹션의 System Logging 3 옵션:

- `log_every_n_steps` (default 10, ge=1): step progress 출력 간격
- `memory_history` (default false): CUDA memory snapshot을 `storage/memory_profiles/{run_id}.pickle`로 저장
- `verbose` (default false): 위 자동 설정 모두 해제 (디버깅 전용)

환경변수 `MDP_LOG_VERBOSE=1`로도 verbose 활성 가능(recipe 값과 OR 결합). Warning 화이트리스트 추가는 `mdp/utils/logging.py::WARNING_SUPPRESS_PATTERNS` 리스트에 regex append(code review 필수).

- System Logging 상세 규약 (배너 포맷, step progress 필드, OOM summary 포맷, strategy 조건부 로깅 규칙, 화이트리스트 관리 원칙): [`docs/training.md §System Logging`](docs/training.md#system-logging)
- nohup 운영 가이드 (배너 grep 패턴, verbose 모드, `memory_history` snapshot 워크플로우, OOM 드릴다운): [`docs/infra.md §nohup 환경 모니터링`](docs/infra.md#nohup-환경-모니터링)
...
mdp 자체 로그(`logger = logging.getLogger(__name__)`)는 영향받지 않는다. System Logging의 `setup_logging()`이 CLI 진입 직후 추가로 `httpx`/`urllib3` 다운그레이드 + rank-0 filter + tqdm 단일화를 적용한다 — 두 경로는 중복 없이 보완 관계.
