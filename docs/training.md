...
학습 CLI의 상위 흐름은 `SettingsPlan -> AssemblyPlan -> ExecutionEngine`으로
고정되어 있다.

```
mdp train / mdp rl-train
  -> SettingsPlanner.load_training(...)
  -> RuntimeLauncher
  -> runtime worker
  -> ExecutionEngine.run(settings_plan)
  -> AssemblyPlanner.from_settings_plan(...)
  -> AssemblyMaterializer(...)
  -> Trainer.from_bundle(...).train()
     또는 RLTrainer.from_bundle(...).train()
```

`SettingsPlan`은 env 치환, override, validation scope, callback config,
command intent, distributed intent가 확정된 설정 계획이다. 기존
`SettingsFactory.for_training()`은 호환 facade로 유지되며 planner가 만든
`plan.settings`를 반환한다.
...
`ExecutionEngine`은 SFT/RL dispatch의 소유자다. `_torchrun_entry.run_training()`
은 호환 wrapper로 남아 engine에 위임하고, trainer 생성자는 bundle-oriented
내부 경로(`from_bundle`)와 기존 직접 생성 경로를 함께 유지한다.
...
1. Recipe + Config 로드 -> `SettingsPlan` 생성
...
Legacy compatibility: `manifest.json`이 없는 기존 checkpoint도 best-effort로 읽는다. 이 경우 `trainer_state.json`과 `scaler.pt`만 공통 I/O 계층에서 복원하고, 모델/optimizer/scheduler weight 주입은 기존 trainer resume 경로의 파일명 관례(`model.pt`, `model.safetensors`, `optimizer.pt`, `scheduler.pt`)를 따른다.
