...
`RunPlan`은 env 치환, override, validation scope, callback config,
command intent, distributed intent가 확정된 설정 계획이다.
`SettingsLoader.load_training_settings()`은 stable path-oriented facade로 유지되며
planner가 만든 `plan.settings`를 반환한다. runtime source of truth는
`RunPlan`이다.
...
`mdp.runtime.training.run_training()`가 Settings에서 RunPlan을
만들고 engine에 진입하는 current-process helper이며, `_torchrun_entry.run_training()`
은 torchrun worker adapter로서 이 runtime helper에 위임한다. 공식 runtime
