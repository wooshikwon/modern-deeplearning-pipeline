...
- loss 함수는 별도 폴더 없이 `rl_losses.py` 한 파일에 (알고리즘당 15~30줄)
...
mdp/
  training/
    ...
    rl_losses.py            # DPO, weighted-NTP, GRPO, PPO loss 함수 + 공유 유틸
...
**신규 파일 2개**: `rl_trainer.py`, `rl_losses.py`, `rl_train.py`
...
training/rl_losses.py    → dpo_loss() (~15줄)
```
...
같은 패턴. RL이 파일 1개 더 있을 뿐 (rl_losses.py).
...
| 1-4 | rl_losses.py (compute_log_probs, dpo_loss) | `training/rl_losses.py` |
...
DPO 인프라 위에 critic forward + GAE + AWR + weighted CE 추가. `rl_losses.py`에 `weighted_ntp_loss` 함수 추가.

### Phase 3: GRPO

Generation 루프 추가. `rl_trainer.py`에 generation step. `rl_losses.py`에 `grpo_loss`.
