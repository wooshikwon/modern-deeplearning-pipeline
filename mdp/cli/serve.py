...
from mdp.cli.output import build_error, build_result, emit_result, is_json_mode, resolve_model_source_plan

logger = logging.getLogger(__name__)


def _resolve_serving_runtime_options(
    serving_config,
    *,
    device_map: str | None,
    max_memory: str | None,
) -> tuple[str | None, dict[str, str] | None]:
    """Resolve serving runtime options with artifact config < explicit CLI."""
    ...
    return effective_device_map, effective_max_memory


def _apply_serving_runtime_options(settings, device_map, max_memory):
    from mdp.settings.schema import ServingConfig

    serving_config = settings.config.serving or ServingConfig()
    serving_config.device_map = device_map
    serving_config.max_memory = max_memory
    settings.config.serving = serving_config
...
def run_serve(
    ...
    try:
        source_plan = resolve_model_source_plan(run_id, model_dir, "serve")
        source_dir = source_plan.path
    ...
    try:
        ...
        effective_device_map, effective_max_memory = _resolve_serving_runtime_options(
            serving_config,
        ...
        model.eval()
        serving_config = _apply_serving_runtime_options(
            settings,
