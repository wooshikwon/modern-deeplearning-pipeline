...
`BaseModel`을 상속하여 `forward()`와 `_block_classes`를 선언한다. 반복 블록이 없는 단순 모델은 `_block_classes = None`을 둔다. SFT loss는 `loss:` 외부 criterion을 쓰거나, `forward()` output에 Tensor `loss`를 넣어 제공한다.
...
class MyModel(BaseModel):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        """순전파. external loss를 쓰면 logits, loss 생략이면 loss도 반환."""
        x = self.encoder(batch["pixel_values"])
        logits = self.head(x)
        out = {"logits": logits}
        if "labels" in batch:
            out["loss"] = nn.functional.cross_entropy(logits, batch["labels"])
        return out

    def validation_step(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        """선택 메서드. 없으면 Trainer fallback이 forward loss/logits를 사용."""
...
class ValueModel(BaseModel):
    ...
    # forward()에서 labels가 있으면 {"loss": ...}를 함께 반환한다.
```
...
- `loss:` 생략 → `model.forward(batch)` 호출 → `outputs["loss"]`를 train loss로 사용

HuggingFace AutoModelFor\* 모델은 labels가 있으면 `outputs.loss`를 반환하므로 `loss:` 생략이 기본이다. 커스텀 모델은 `loss:`를 지정해 supervised criterion을 외부화하거나, `forward()`에서 labels가 있을 때 `loss`를 반환한다.

raw timm/torchvision-style vision 모델은 보통 `forward(x) -> logits`만 제공한다. 이 경우 `loss:`를 지정하면 MDP가 `batch["pixel_values"]`를 `x`로 전달하고 external loss가 `logits`와 `labels`로 학습한다. `loss:`를 생략하려면 wrapper를 작성해 `forward()` output에 Tensor `loss`를 넣어야 한다.

### Dict I/O 규칙

모든 입출력은 dict 기반이다:
- **입력**: Collator가 생성한 표준 키 (`input_ids`, `pixel_values`, `labels` 등)
- **출력**: 외부 `loss:` 사용 시 최소 `"logits"` 키 필요. `loss:` 생략 시 Tensor `"loss"` 키 필요
...
trainer는 모델의 custom 메서드를 분산 래퍼 위에서 호출할 때 아래 두 메서드를 통해 dispatch한다. 모델 구현자는 분산 래퍼를 의식하지 않고 plain 메서드를 선언하면 되고, 커스텀 전략은 필요에 따라 이 메서드들만 override하면 된다.
